...
const Experience = () => {
  return (
    <section id="experience" className="scroll-mt-20 p-4">
      <h1 className="sm:text-xl text-[#ccc] font-bold mb-4">Experience</h1>
      <div>
        {exp.map((ex, index) => (
          <div key={index} className="mt-6 pb-6 p-4 bg-[#1f1f1f] rounded-2xl shadow-sm">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 gap-2">
              <div>
                <h1 className="text-xl sm:text-2xl font-extrabold">{ex.company}</h1>
                <h5 className="text-sm sm:text-base text-gray-400">{ex.title}</h5>
              </div>
              <label className="text-xs sm:text-sm text-gray-300">{ex.date}</label>
            </div>
            <p className="tracking-wide text-sm sm:text-base bg-[#282828] text-gray-200 p-4 border border-gray-600 rounded-xl leading-relaxed">
              {ex.desc}
            </p>
          </div>
        ))}
